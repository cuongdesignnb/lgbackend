...
function runBatch(id) {
    if (!confirm('Bạn có chắc chắn muốn chạy batch này?')) return;
    router.post(`/admin/ai-articles/${id}/run`, {}, { preserveScroll: true });
}

function deleteBatch(id) {
    if (!confirm('Bạn có chắc chắn muốn xóa batch này?')) return;
...
<template>
<AdminLayout title="AI Bài viết">
    <div class="max-w-6xl">
        <!-- Header -->
        <div class="flex items-center justify-between mb-6">
            <div>
                <h3 class="text-lg font-bold text-slate-200">AI Bài viết - Tạo hàng loạt</h3>
                <p class="text-xs text-slate-500 mt-1">Tạo bài viết tự động từ danh sách từ khóa bằng AI</p>
            </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-slate-800/40">
                            <th class="px-5 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Tên batch</th>
                            <th class="px-5 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Provider</th>
                            <th class="px-5 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Tiến độ</th>
                            <th class="px-5 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Trạng thái</th>
                            <th class="px-5 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Lịch hẹn</th>
                            <th class="px-5 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Tự động đăng</th>
                    ...
                    <tbody class="divide-y divide-slate-800/30">
                        <tr v-for="batch in batches?.data || []" :key="batch.id" class="hover:bg-slate-800/30 transition-colors">
                            <td class="px-5 py-3">
                                ...
                                <p class="text-xs text-slate-500 mt-0.5">{{ batch.total_items }} từ khóa</p>
                            </td>
                            ...
                            <td class="px-5 py-3 text-center">
                                <span v-if="batch.auto_publish" class="text-xs text-emerald-400">Có</span>
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <Link :href="`/admin/ai-articles/${batch.id}`" class="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded transition-colors" title="Chi tiết">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </Link>
                                    <button @click="deleteBatch(batch.id)" class="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors" title="Xóa">
                        ...
                        <tr v-if="!batches?.data?.length">
                            <td colspan="7" class="px-5 py-10 text-center text-sm text-slate-600">Chưa có batch nào. Bấm "Tạo batch mới" để bắt đầu.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
