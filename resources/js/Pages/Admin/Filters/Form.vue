...
<AdminLayout :title="isEdit ? `Sửa bộ lọc: ${filter.name}` : 'Thêm bộ lọc'">
    <div class="max-w-4xl">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold text-gray-900">{{ isEdit ? `Sửa: ${filter.name}` : 'Thêm bộ lọc mới' }}</h3>
            <Link href="/admin/filters" class="text-sm text-gray-500 hover:text-gray-700">← Quay lại</Link>
        </div>
        <form @submit.prevent="submit" class="space-y-6">
            <!-- Thông tin bộ lọc -->
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
                <h4 class="text-sm font-semibold text-gray-700 uppercase tracking-wider">Thông tin bộ lọc</h4>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tên bộ lọc *</label>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Loại bộ lọc</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tìm kiếm trong</label>
                        <select v-model="form.match_field" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    ...
                    <div>
                        ...
                        <input v-model="form.sort_order" type="number" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                    </div>
                </div>
                <label class="flex items-center gap-2 text-sm">
                    <input v-model="form.is_active" type="checkbox" class="rounded border-gray-300 text-indigo-600"> Kích hoạt
                </label>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
                <div class="flex items-center justify-between">
                    <h4 class="text-sm font-semibold text-gray-700 uppercase tracking-wider">
                        Giá trị bộ lọc <span class="text-gray-400 font-normal normal-case">({{ form.values.length }} giá trị)</span>
                    </h4>
                ...
                <div v-if="form.values.length" class="border border-gray-200 rounded-lg overflow-hidden">
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase w-8">#</th>
                                <th class="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase">Nhãn hiển thị</th>
                                <th class="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase">Slug (URL)</th>
                                <th v-if="form.match_field !== 'price'" class="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase">
                                    Giá trị tìm kiếm
                                </th>
                                <th v-if="form.match_field === 'price' || form.type === 'price_range'" class="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase">Giá min</th>
                                <th v-if="form.match_field === 'price' || form.type === 'price_range'" class="px-3 py-2.5 text-left text-xs font-semibold text-gray-600 uppercase">Giá max</th>
                                <th class="px-3 py-2.5 text-center text-xs font-semibold text-gray-600 uppercase w-16">Bật</th>
                                <th class="px-3 py-2.5 text-center text-xs font-semibold text-gray-600 uppercase w-24">Thao tác</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="(val, idx) in form.values" :key="idx" class="hover:bg-gray-50/50">
                                ...
                                <td class="px-3 py-2">
                                    <input v-model="val.label" @blur="onValueLabelBlur(val)" placeholder="VD: Intel Core i5"
                                        class="w-full border border-gray-300 rounded px-2.5 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500">
                                </td>
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2 text-center">
                                    <input v-model="val.is_active" type="checkbox" class="rounded border-gray-300 text-indigo-600">
                                </td>
                                <td class="px-3 py-2 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        <button type="button" @click="moveValue(idx, -1)" :disabled="idx === 0"
                                            class="p-1 text-gray-400 hover:text-gray-600 disabled:opacity-30" title="Lên">↑</button>
                                        <button type="button" @click="moveValue(idx, 1)" :disabled="idx === form.values.length - 1"
                                            class="p-1 text-gray-400 hover:text-gray-600 disabled:opacity-30" title="Xuống">↓</button>
            ...
            <div class="flex justify-end gap-3">
                <Link href="/admin/filters" class="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg">Hủy</Link>
