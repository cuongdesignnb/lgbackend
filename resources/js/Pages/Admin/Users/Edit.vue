...
const form = useForm({
    ...
    spatie_roles: props.editUser.spatie_roles || [],
});
...
<template>
    <div class="max-w-2xl">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl font-bold text-slate-100">Sửa tài khoản: {{ editUser.name }}</h1>
        </div>
        ...
        <form @submit.prevent="submit" class="bg-slate-900/60 border border-slate-800/60 rounded-xl p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1.5">Mật khẩu mới <span class="text-slate-500">(để trống nếu không đổi)</span></label>
                    <input v-model="form.password" type="password" class="w-full px-4 py-2.5 bg-slate-800/60 border border-slate-700/50 rounded-lg text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-cyan-500/50" />
                    <p v-if="form.errors.password" class="text-xs text-red-400 mt-1">{{ form.errors.password }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1.5">Xác nhận mật khẩu mới</label>
                    <input v-model="form.password_confirmation" type="password" class="w-full px-4 py-2.5 bg-slate-800/60 border border-slate-700/50 rounded-lg text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-cyan-500/50" />
                </div>
            </div>
