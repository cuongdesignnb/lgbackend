...
<template>
<AdminLayout title="Thêm tài liệu">
    <div class="max-w-3xl">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold text-slate-200">Thêm tài liệu kỹ thuật</h3>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="bg-slate-900 rounded-lg border border-slate-800/60 p-6">
                <h4 class="text-sm font-semibold text-slate-300 mb-4">File tài liệu <span class="text-red-400">*</span></h4>
                <div v-if="!filePreview && !form.file_url" class="border-2 border-dashed border-slate-700/50 rounded-lg p-8 text-center hover:border-cyan-500/50 transition-colors">
                    ...
                    <label class="cursor-pointer"><span class="px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 text-sm font-medium inline-block">Chọn file từ máy</span><input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.zip" @change="onFileChange" class="hidden" /></label>
                    <p class="text-slate-500 text-xs mt-3">PDF, DOC, XLS, ZIP — tối đa 50MB</p>
                    <div class="mt-3"><span class="text-slate-600 text-xs">hoặc nhập URL trực tiếp:</span></div>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" :disabled="form.processing" class="px-6 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 text-sm font-medium disabled:opacity-50">{{ form.processing ? 'Đang tạo...' : 'Tạo tài liệu' }}</button>
            </div>
        </form>
    </div>
</AdminLayout>
